...
class SimHubb2P:
    ...
    def hamiltonian(self):
        ...
        for i in range(L):
            for delta in [-1, 1]:  # nearest neighbors
                j = i + delta
                if self.periodic:
                    j = j % L  # Handle periodic boundaries
                elif 0 <= j < L:  # Open boundaries
                    single_hop[i,j] = -t
                # else: stays zero (open boundary)

        # Two-particle kinetic term
        H_kin = np.kron(single_hop, np.eye(L)) + np.kron(np.eye(L), single_hop)

        H_int = np.zeros((L*L, L*L))
        np.fill_diagonal(H_int, [U if i//L == i%L else 0 for i in range(L*L)])
    ...
    def plot_eigenstate(self, state_idx=0):
        ...
        if state_idx >= max_states:
            raise ValueError(f"state_idx must be < {max_states} (L²) for L={self.L}")
        ...
        ax1.bar(sites - 0.2, prob_up, width=0.4, label='Up electron', alpha=0.7)
        ax1.bar(sites + 0.2, prob_down, width=0.4, label='Down electron', alpha=0.7)
        ax1.set_xlabel('Lattice site')
        ax1.set_ylabel('Probability density')
        ax1.set_title(f'Individual particle probabilities\n(Eigenstate {state_idx})')
        ax1.legend()
